...
def denoise_complete_movie(
    ...
    """
    Applies deep interpolation to denoise a complete movie. 

    Parameters
    ----------
    mov : numpy.ndarray
        A 3D numpy array representing the input movie to be denoised
    timestamps : numpy.ndarray
        A 1D numpy array representing the timestamps for each frame in the movie
    output_dir : str
        The path to the output directory where the denoised movie will be saved
    run_uid : str
        A unique identifier for this denoising run
    resize : str, optional
        A string indicating the type of resizing to use, by default 'full'
    size : int, optional
        An integer indicating the size of the resized movie, by default 256
    steps_per_epoch : int, optional
        An integer indicating the number of steps per epoch during model training, by default 10
    pre_frame : int, optional
        An integer indicating the number of frames to use before the current frame during denoising, by default 30
    post_frame : int, optional
        An integer indicating the number of frames to use after the current frame during denoising, by default 30
    batch_size : int, optional
        An integer indicating the batch size to use during training and inference, by default 5
    test_start_frame : int, optional
        An integer indicating the index of the first frame to use for testing, by default 0
    test_end_frame : int, optional
        An integer indicating the index of the last frame to use for testing, by default 1000
    train_start_frame : int, optional
        An integer indicating the index of the first frame to use for training, by default 0
    train_end_frame : int, optional
        An integer indicating the index of the last frame to use for training, by default -1
    lr : float, optional
        A float indicating the learning rate to use during model training, by default 0.0001
    loss : str, optional
        A string indicating the loss function to use during model training, by default 'mean_absolute_error'
    load_previous_training : bool, optional
        A boolean indicating whether to load a previously trained model for further training, by default False
    load_previous_inference : bool, optional
        A boolean indicating whether to load a previously generated inference file for further inference, by default False

    Returns
    -------
    numpy.ndarray
        A 3D numpy array representing the denoised movie
    numpy.ndarray
        A 1D numpy array representing the timestamps for each frame in the denoised movie
    """

    assert DEEP_INTERP, 'deep interpolation library not installed. Installation necessary for this function'
    
    print('resizing image for denoising')
    mov, nshape, oshape = resize_image(mov, resize, size)
    
    print('saving tif file for denoising')
    tfilename = "./temp_mc_movie.tif"
    tifffile.imwrite(tfilename, mov)
    ...
    model = (
        ...
        "padding_unet_single_1024"
    )
    print('initialize training')
    if load_previous_training:
        filepaths = glob(os.path.join(output_dir, f'*_{model}_model.h5'))
        if filepaths:
            # take latest model
            filepath = np.sort(filepaths)[-1]
            # replace run_uid for inference
            run_uid = filepath.replace(f'_{model}_model.h5', '')
        else:
            raise ValueError("No previous training to load")
    else:
        deepinterp_training(
            tfilename, 
            model,
    ...
    print("Infer movie")
    mov, timestamps = deepinterp_inference(
        tfilename,
        model,
        output_dir,
        run_uid, 
    ...
    print("resize movie")
    mov = size_image(mov, nshape, oshape)
    
    delete_filenames(tfilename)
    return mov, timestamps
...
def deepinterp_training(
    ...
    """
    Trains a deep learning model on a single TIF file using a generator and a specified set of parameters

    Parameters
     ----------
    filename: path to the TIF file
    model:  name of the deep learning model to use
    output_dir: directory where the trained model will be saved
    run_uid: unique identifier for this training run
    steps_per_epoch: number of steps to take per epoch
    pre_frame: number of frames to include before the target frame in the training data
    post_frame: number of frames to include after the target frame in the training data
    batch_size: number of samples per batch
    test_start_frame: index of the first frame to use for testing
    test_end_frame: index of the last frame to use for testing
    train_start_frame: index of the first frame to use for training
    train_end_frame: index of the last frame to use for training
    ...
    generator_test_param["data_path"] = filename
    generator_test_param["batch_size"] = batch_size
    generator_test_param["start_frame"] = test_start_frame
    generator_test_param["end_frame"] = test_end_frame
    ...
    generator_param["data_path"] = filename
    generator_param["batch_size"] = batch_size
    generator_param["start_frame"] = train_start_frame
    generator_param["end_frame"] = train_end_frame
    generator_param["pre_post_omission"] = 0

    # Those are parameters used for the network topology
    network_param["name"] = model

    # Those are parameters used for the training process
    training_param["name"] = "core_trainer"
    training_param["learning_rate"] = lr
    training_param["loss"] = loss # "mean_squared_error"
    training_param["output_dir"] = output_dir
    training_param['steps_per_epoch'] = steps_per_epoch

    training_param["model_string"] = model
...
def deepinterp_inference(
    ...
    """
    Performs inference on a trained deep learning model for a given input file

    Parameters
    ----------
        filename: path to the input file
        model: name of the trained deep learning model
        output_dir: directory where the output will be stored
        run_uid: unique identifier for the current run
        timestamps: array of timestamps corresponding to the input file
        pre_frame: number of frames to use as input before the current frame
        post_frame: number of frames to use as input after the current frame
        batch_size: number of frames to process in each batch
        load_previous_inference: boolean value indicating whether to load the results of a previous inference if available
    
    Returns 
    -------
    The movie (a numpy array) and the timestamps as a tuple

    """

    output_file = os.path.join(output_dir, f"{run_uid}_inference_output.h5")
    
    if load_previous_inference:
        if not os.path.exists(output_file):
            raise ValueError("Cannot load previous inference")
    ...
    # actually perform the inference
        ...
        generator_param["batch_size"] = batch_size  # Increasing this number will facilitate
        ...
        local_path = os.path.join(output_dir, f"{run_uid}_{model}_model.h5")
        assert os.path.exists(local_path), f"Model does not exist:\n{glob.glob(os.path.join(output_dir, '*model.h5'))}"

        inference_param["model_source"] = {"local_path": local_path}
    ...
    print("loading inferred movie")
    # load denoised movie and format correctly
    with h5py.File(output_file, "r") as f:
        mov = np.array(f['data'])
    
    timestamps = timestamps[
        pre_frame:
        mov.shape[0]+pre_frame
    ]
    assert len(timestamps) == len(mov), f"Lengths of timestamps and movie do not match; BUG: {len(timestamps)} AND {len(mov)}"
    return mov, timestamps
